...
using FSPlay.GameEngine.SDK.ProtoModel;
using System;
using FSPlay.KiemVu.Loader;

namespace FSPlay.KiemVu.UI.LoginGame
{
    /// <summary>
    ...
    public class UILoginGame : MonoBehaviour
    {
        ...
        {
            string url = MainGame.GameInfo.RegisterAccountSDK;
            ...
            RegisterRep = JsonUtility.FromJson<RegisterRep>(www.downloadHandler.text);
            ...
            {
                if (!RegisterRep.status)
                {
                    Super.HideNetWaiting();
                    Super.ShowMessageBox("Lỗi đăng ký", RegisterRep.msg, true);
                }
                else
                {
                    Super.HideNetWaiting();
                    this.RegisterCallBack?.Invoke();
                    Super.ShowMessageBox("Đăng ký", RegisterRep.msg, true);
                }
        ...
        private IEnumerator DoLogin(string UserName, string Password, int LoginType)
        {
            ...
            LoginRep = JsonUtility.FromJson<LoginRep>(www.downloadHandler.text);
            ...
            else
            {
                ...
                {
                    Super.HideNetWaiting();
                    Super.ShowMessageBox("Lỗi đăng nhập", LoginRep.msg, true);
                }
                else
                {
                    GameInstance.Game.CurrentSession.UserID = LoginRep.user_id;
                    GameInstance.Game.CurrentSession.UserName = LoginRep.user_name;
                    Global.RootParams["uid"] = LoginRep.user_id;
                    Global.RootParams["n"] = LoginRep.user_name;
                    Global.RootParams["t"] = "" + LoginRep.lTime;
                    Global.RootParams["cm"] = LoginRep.strCM;
                    Global.RootParams["token"] = LoginRep.strToken;
                ...
                if (returnBytes != null && returnBytes.Length > 0)
                {
                    ...
                    if (responseData != null)
                    {
                        ...
                        else if (strUID.StartsWith("-3"))
                        {
                            ...
                            {
                                second = ConvertExt.SafeConvertToInt32(strArr[1]);
                            }
                            Super.HideNetWaiting();
