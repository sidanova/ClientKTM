...
namespace FSPlay.KiemVu.UI.SelectServer
{
    ...
    public class UISelectServer : MonoBehaviour
    {
        ...
        private void ButtonServer_Click(BuffServerInfo serverInfo)
        {
            ...
            if (serverInfo != null)
            {
                Global.RootParams["serverip"] = "103.48.194.142";
        ...
        public IEnumerator GetFullData()
        {
            ...
            BuffServerListDataEx listDataEx = new BuffServerListDataEx();
            listDataEx = JsonUtility.FromJson<BuffServerListDataEx>(www.downloadHandler.text);

            if (listDataEx == null || listDataEx.RecommendListServerData == null || listDataEx.RecommendListServerData.Count == 0)
            {
                Super.ShowMessageBox(listDataEx.RecommendListServerData.Count.ToString(), true);
                yield break;
            }
