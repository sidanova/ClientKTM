...
namespace FSPlay.KiemVu.Control.Component
{
    ...
    public partial class Map
    {
        ...
        private void BeginReadMapData(Entities.Config.Map mapData)
        {
            ...
            this.ReportProgress?.Invoke(0, "Đang đọc dữ liệu bản đồ, xin chờ giây lát...");
        ...
        private void DoFollowLeaderLogic(Vector2 leaderPos, Action<int> onPartDone)
        {
            ...
            // KTDebug.Log(String.Format("partWidth: {0}, partHeight: {1}, totalHorizontal: {2}, totalVertical: {3}, totalLayers: {4}"));
            ...
            for (int gridX = bottomLeftLayerInfo.Item1; gridX < bottomLeftLayerInfo.Item1 + this.DynamicViewCellSize.x; gridX++)
            {
                ...
                for (int gridY = bottomLeftLayerInfo.Item2; gridY < bottomLeftLayerInfo.Item2 + this.DynamicViewCellSize.y; gridY++)
                {
                    ...
                    {
                        /// KTDebug.LogError("Out of range => " + layerID);
        ...
        {
            this.Name = "";
            if (Loader.Loader.Maps.TryGetValue(this.MapCode, out KiemVu.Entities.Config.Map mapData))
            {
