...
namespace FSPlay.KiemVu.Entities
{
    ...
    public class GameInfo
    {
        ...
        #region Loading Resource Screen
        /// <summary>
        /// Tên File ảnh màn hình tải dữ liệu
        /// </summary>
        public string LoadingScreenFileName { get; set; }

        /// <summary>
        /// Checksum File ảnh màn hình tải dữ liệu
        /// </summary>
        public string LoadingScreenMD5Checksum { get; set; }
        #endregion
        ...
        {
            string strCdnUrl = xmlNode.Element("CdnUrl").Attribute("Url").Value;
            string[] listCdnUrl = strCdnUrl.Split(',');
            int keyCdnUrl = UnityEngine.Random.Range(0, listCdnUrl.Length);

            return new GameInfo()
            {
                URL = xmlNode.Attribute("URL").Value,
                CdnUrl = listCdnUrl[keyCdnUrl],
                ...
                LoadingScreenFileName = xmlNode.Element("LoadingResourceScreen").Attribute("FileName").Value,
                LoadingScreenMD5Checksum = xmlNode.Element("LoadingResourceScreen").Attribute("MD5").Value,
        ...
        public XElement ToXML()
        {
            ...
            XElement loadingScreenNode = new XElement("LoadingResourceScreen");
            loadingScreenNode.Add(new XAttribute("FileName", this.LoadingScreenFileName));
            loadingScreenNode.Add(new XAttribute("MD5", this.LoadingScreenMD5Checksum));
            xmlNode.Add(loadingScreenNode);
